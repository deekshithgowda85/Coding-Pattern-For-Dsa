...
vector<double> findAvgOfSubarrays(vector<int>& arr, int K) {
...
vector<double> findAveragesOfSubarrays(vector<int>& arr, int k) {
...
// Example: findAveragesOfSubarrays([1, 3, 2, 6, -1, 4, 1, 8, 2], 5) -> [2.2, 2.8, 2.4, 3.6, 2.8]
```
...
int maxSubarrayOfSizeK(vector<int>& arr, int k) {
  //brute force
  int maxSum = 0;
  int windowSum = 0;
...
int smallestSubarrayWithGivenSum(vector<int>& arr, int s) {
...
int longestSubstringWithKdistinct(string str, int k) {
  ...
  for(int windowEnd = 0; windowEnd < str.length(); windowEnd++) {
    ...
    while(charFrequency.size() > k) {
      ...
      //while shrinking, we will decrement the character's frequency going out of the window
...
int main() {
    vector<int> fruits1 = {3, 3, 3, 1, 2, 1, 1, 2, 3, 3, 4};
    cout << totalFruit(fruits1) << endl;  // 5

    vector<int> fruits2 = {1, 2, 1};
    cout << totalFruit(fruits2) << endl;  // 3, We can pick from all 3 trees.

    vector<int> fruits3 = {0, 1, 2, 2};
    cout << totalFruit(fruits3) << endl;  // 3, We can pick from trees [1,2,2].

    vector<int> fruits4 = {1, 2, 3, 2, 2};
    cout << totalFruit(fruits4) << endl;  // 4, We can pick from trees [2,3,2,2].
...
int fruitsInBaskets(vector<string>& fruits) {
  int windowStart = 0;
  int maxLength = 0;
  unordered_map<string, int> fruitFrequency;

  //try to extend the range
  for (int windowEnd = 0; windowEnd < fruits.size(); windowEnd++) {
    string endFruit = fruits[windowEnd];
    ...
    while (fruitFrequency.size() > 2) {
      string startFruit = fruits[windowStart];
      fruitFrequency[startFruit]--;
      if (fruitFrequency[startFruit] == 0) {
        fruitFrequency.erase(startFruit);
      }
      windowStart++;
    }
    maxLength = max(maxLength, windowEnd - windowStart + 1);
...
int nonRepeatSubstring(string str) {
  ...
  for (int windowEnd = 0; windowEnd < str.length(); windowEnd++) {
    ...
    if (charIndexMap.find(endChar) != charIndexMap.end()) {
...
int lengthOfLongestSubstring(string str, int k) {
  int windowStart = 0;
  int maxLength = 0;
...
int lengthOfLongestSubstring(vector<int>& arr, int k) {
  int windowStart = 0;
  int maxLength = 0;
  int maxOnesCount = 0;
...
bool findPermutation(string str, string pattern) {
  ...
  for (int windowEnd = 0; windowEnd < str.length(); windowEnd++) {
    char endChar = str[windowEnd];
    if (charFrequency.find(endChar) != charFrequency.end()) {
    ...
    if (windowEnd >= pattern.length() - 1) {
      char startChar = str[windowStart];
      windowStart++;
      if (charFrequency.find(startChar) != charFrequency.end()) {
        if (charFrequency[startChar] == 0) {
          isMatch--;
        }
        charFrequency[startChar]++;
...
vector<int> findStringAnagrams(string str, string pattern) {
  int windowStart = 0;
  int matched = 0;
  unordered_map<char, int> charFreq;
  ...
  for (int windowEnd = 0; windowEnd < str.length(); windowEnd++) {
    char endChar = str[windowEnd];
    if (charFreq.find(endChar) != charFreq.end()) {
    ...
    if (windowEnd >= pattern.length() - 1) {
      char startChar = str[windowStart];
      windowStart++;
      if (charFreq.find(startChar) != charFreq.end()) {
        if (charFreq[startChar] == 0) {
...
vector<int> findWordConcatenation(string str, vector<string>& words) {
