...
int main() {
  vector<Interval> merged = merge({{1, 4}, {2, 5}, {7, 9}});
  for (const auto& it : merged) {
    cout << it.getInterval() << " ";
  }
  cout << "\n";

  merged = merge({{6, 7}, {2, 4}, {5, 9}});
  for (const auto& it : merged) {
    cout << it.getInterval() << " ";
  }
  cout << "\n";

  merged = merge({{1, 4}, {2, 6}, {3, 5}});
  for (const auto& it : merged) {
    cout << it.getInterval() << " ";
  }
  cout << "\n";
...
bool anyOverlap(vector<vector<int>> intervals) {
 if (intervals.size() < 2) {
   return false;
 }
...
vector<vector<int>> findIntersection(const vector<vector<int>>& firstIntervals,
                                     const vector<vector<int>>& secondIntervals) {
  vector<vector<int>> result;
  size_t i = 0;
  size_t j = 0;

  while (i < firstIntervals.size() && j < secondIntervals.size()) {
    int start1 = firstIntervals[i][0];
    int end1 = firstIntervals[i][1];
    int start2 = secondIntervals[j][0];
    int end2 = secondIntervals[j][1];

    bool overlap = (start1 >= start2 && start1 <= end2) || (start2 >= start1 && start2 <= end1);
    if (overlap) {
...
bool canAttendAllAppointments(vector<vector<int>> appointments) {
  sort(appointments.begin(), appointments.end(), [](const vector<int>& a, const vector<int>& b) {
    return a[0] < b[0];
  });

  for (size_t i = 1; i < appointments.size(); i++) {
...
vector<vector<vector<int>>> whatAreTheConflicts(const vector<vector<int>>& appointments) {
  vector<vector<vector<int>>> conflicts;
  for (size_t i = 0; i < appointments.size(); i++) {
    for (size_t j = i + 1; j < appointments.size(); j++) {
      if (appointments[i][1] > appointments[j][0] && appointments[j][1] > appointments[i][0]) {
        conflicts.push_back({appointments[i], appointments[j]});
      }
    }
  }
  return conflicts;
...
int findMaxCPULoad(vector<vector<int>> jobs) {
  sort(jobs.begin(), jobs.end(), [](const vector<int>& a, const vector<int>& b) {
    return a[0] < b[0];
  });

  using Job = pair<int, int>;  // end, load
  priority_queue<Job, vector<Job>, greater<Job>> minHeap;
  int currentLoad = 0;
  int maxLoad = 0;
...
vector<vector<int>> findEmployeeFreeTime(const vector<vector<vector<int>>>& schedules) {
  vector<vector<int>> allTime;
  for (const auto& emp : schedules) {
    for (const auto& slot : emp) {
      allTime.push_back(slot);
    }
  }
  sort(allTime.begin(), allTime.end(), [](const vector<int>& a, const vector<int>& b) {
    return a[0] < b[0];
  });

  vector<vector<int>> merged;
  merged.push_back(allTime[0]);
  for (size_t i = 1; i < allTime.size(); i++) {
    if (allTime[i][0] <= merged.back()[1]) {
      merged.back()[1] = max(merged.back()[1], allTime[i][1]);
    } else {
      merged.push_back(allTime[i]);
    }
  }
