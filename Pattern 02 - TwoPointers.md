...
// Hash table lookup for the complement of each value.
pair<int, int> pairWithTargetSum(const vector<int>& nums, int target) {
...
void searchPair(vector<int>& arr, int targetSum, int first, vector<vector<int>>& triplets) {
  int start = first + 1;
  int end = arr.size() - 1;
...
vector<vector<int>> findSubarrays(vector<int>& arr, int target) {
  ...
  for(int end = 0; end < arr.size(); end++) {
    ...
    for(int i = end; i > start - 1; i--) {
      ...
      result.push_back(vector<int>(tempList.begin(), tempList.end()));
    }
  }
  return result;
}
...
vector<int> dutchFlagSort(vector<int>& arr) {
...
// dutchFlagSort({1, 0, 2, 1, 0}) // [0 0 1 1 2]
// dutchFlagSort({2, 2, 0, 1, 2, 0}) // [0 0 1 2 2 2 ]
```
...
void searchPairs(vector<int>& arr, int targetSum, int first, int second, vector<vector<int>>& quads) {
  int start = second + 1;
  int end = arr.size() - 1;
...
int getNextChar(string& str, int index) {
